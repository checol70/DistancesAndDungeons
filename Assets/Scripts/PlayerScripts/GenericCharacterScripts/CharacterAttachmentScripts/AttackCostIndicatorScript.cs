...
public class AttackCostIndicatorScript : MonoBehaviour {
    public float
        ViewRadius;
    public LayerMask
        targetMask;
    ...
    private void Start()
    {
        ...
        gameObject.transform.parent.GetComponent<CharacterScript>().AttackCostIndicator = gameObject.transform;
        ViewMesh = new Mesh();
    ...
    {
        if (gameObject.transform.root.gameObject == CameraScript.GameController.ActivePlayer && CameraScript.GameController.PlayerTurn && character.RemainingMovement >= character.AttackCost)
        {
            ViewRadius = character.RemainingMovement - character.AttackCost;
            DrawFieldOfView();
